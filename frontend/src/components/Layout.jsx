    ...
    <div className="layout">
      ...
      <aside className={`sidebar${sidebarOpen ? ' open' : ''}`}>
        <div className="sidebar-logo">
          <img src="/logo.png" alt="SSR Iberia" style={{ height: '38px', marginBottom: '6px', filter: 'brightness(0) invert(1)' }} />
          <span>Sistema de Seguros — Demo</span>
        </div>

        <nav className="sidebar-nav">
          {!isExterno && <div className="sidebar-section">Principal</div>}
          <NavLink to="/" end onClick={closeSidebar} className={({ isActive }) => isActive ? 'active' : ''}>
            <span className="nav-icon">📊</span> Dashboard
          </NavLink>
          <NavLink to="/proyectos" onClick={closeSidebar} className={({ isActive }) => isActive ? 'active' : ''}>
          ...
          {!isExterno && (
            <>
              ...
              <NavLink to="#" style={{ opacity: 0.4, pointerEvents: 'none' }}>
